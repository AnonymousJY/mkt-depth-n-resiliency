...
def plot_figure_3(var_df: pd.DataFrame, point_in_time_dt: str) -> Path:
    ...
    # Paper Figure 3 uses per-panel color scales -- each panel's own
    # (vmin, vmax) is inferred by matplotlib from that panel's VaR range.
    # Sharing vmin/vmax across panels flattens the low-VaR panels to
    # all-blue, which doesn't match the paper. Leave normalization to
    # plot_surface's default (per-panel).
